        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                ...
                <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                        <span className="text-gray-500">Low Pressure</span>
            ...
            {periodComparisons.map((comparison, index) => (
                <div key={comparison.period} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-3">
                            <div className={`p-2 rounded-lg ${index === 0 ? 'bg-teal-100' : index === 1 ? 'bg-purple-100' : 'bg-indigo-100'
                                }`}>
                                <Droplets className={`h-5 w-5 ${index === 0 ? 'text-teal-600' : index === 1 ? 'text-purple-600' : 'text-indigo-600'
                                    }`} />
                            </div>
                            <div>
                                <p className="text-sm font-medium text-gray-600">{comparison.period}</p>
                                <p className="text-2xl font-bold text-gray-900">{comparison.currentUsage.toLocaleString()}L</p>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center space-x-2">
                        ...
                        <span className={`text-sm font-medium ${comparison.changePercent > 0 ? 'text-red-600' : 'text-green-600'
                            }`}>
                    ...
                    <div className="mt-2 text-xs text-gray-500">
                        {comparison.changePercent > 0 ? '+' : ''}{comparison.change.toLocaleString()}L change
                    </div>
                </div>
            ))}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
                ...
                <div className="flex items-center space-x-2 text-sm text-gray-500">
                    ...
                    <span>{new Date(notificationStats.lastAlert).toLocaleTimeString()}</span>
                </div>
            </div>
        </div>
    );
