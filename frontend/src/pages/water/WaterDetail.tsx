...
import { WaterMeterConfirmModal } from '../../components/waterreq/WaterMeterConfirmModal';
...
const WaterMonitoringDashboard: React.FC = () => {
  ...
  const exportToExcel = () => {
    if (
      ...
      waterDetail.DailyWaterUsage.length === 0
    ) {
    ...
    const usageData = sortedUsage.map((item) => {
      const itemDate = item.Timestamp ? new Date(item.Timestamp) : null;

      return {
        วันที่เวลา: itemDate
          ? itemDate.toLocaleString("th-TH", {
            year: "numeric",
            month: "long",
            day: "numeric",
            hour: "2-digit",
          ...
          : "-",
        ปริมาณน้ำที่ใช้: item.Usage ?? "-",
      };
    });

    const usageSheet = XLSX.utils.json_to_sheet(usageData);

    // 🔹 ใส่สีเหลือง + ตัวหนาให้หัวตาราง
    ["A1", "B1"].forEach((cell) => {
      if (usageSheet[cell]) {
        usageSheet[cell].s = {
          fill: { fgColor: { rgb: "FFFF00" } }, // พื้นหลังเหลือง
          font: { bold: true }, // ตัวหนา
          alignment: { horizontal: "center", vertical: "center" }, // จัดกลาง
        };
      }
    });

    // ----------------------
    // 2. Sheet: ค่ามิเตอร์น้ำตามเวลา
    // ----------------------
    const sortedMeters = [...(waterDetail.WaterMeterValue || [])].sort((a, b) => {
      const timeA = a.Timestamp ? new Date(a.Timestamp).getTime() : 0;
      const timeB = b.Timestamp ? new Date(b.Timestamp).getTime() : 0;
      return timeA - timeB;
    });

    const meterData = sortedMeters.map((wmv) => {
      const date = wmv.Timestamp ? new Date(wmv.Timestamp) : null;
      return {
        วันที่เวลา: date
    ...
    XLSX.utils.book_append_sheet(workbook, usageSheet, "ข้อมูลการใช้น้ำ");
    XLSX.utils.book_append_sheet(workbook, meterSheet, "ค่ามิเตอร์น้ำตามเวลา");
    ...
    saveAs(
      blob,
      `ข้อมูลปริมาณการใช้น้ำ_${waterDetail.MeterLocation?.Name || "Unknown"}.xlsx`
    );
  };
  ...
  const getMeterLocationDetailById = async (startDate?: string, endDate?: string) => {
    try {
      ...
      if (res.status === 200) {
        ...
        messageApi.success("โหลดข้อมูลสำเร็จ");
  ...
  const handleVerify = async (id: number, meterValue: number) => {
    try {
      await updateWaterValueStatusById(id.toString(), meterValue);
      loadWaterValueReq();
      setIsImageModalOpen(false);

      messageApi.success("ยืนยันข้อมูลสำเร็จแล้ว");
    } catch (error) {
      console.error(`Verify failed for ID ${id}`, error);
      messageApi.error("เกิดข้อผิดพลาดในการยืนยันข้อมูล");
    }
  };
  const handleReject = async (id: number) => {
    try {
      await updateWaterValueStatusToReJect(id.toString());
      loadWaterValueReq();
      setIsImageModalOpen(false);

      messageApi.success("ปฏิเสธข้อมูลสำเร็จแล้ว");
    } catch (error) {
      console.error(`Reject failed for ID ${id}`, error);
      messageApi.error("เกิดข้อผิดพลาดในการปฏิเสธข้อมูล");
    }
  };
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-lg mb-6 p-4 border border-white/20">
          <div className="flex flex-col space-y-3 sm:space-y-0 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-row items-center gap-4">
              <button
                onClick={() => navigate(-1)}
                className="flex items-center gap-2 px-4 py-2 bg-white/80 backdrop-blur-sm text-gray-700 rounded-xl shadow-md hover:shadow-lg hover:bg-white transition-all duration-300 border border-gray-200 cursor-pointer"
              >
                  ...
                  <LineChart data={dailyMeterData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                    ...
                    {avgValue != null && (
                      <ReferenceLine
                        ...
                        strokeDasharray="5 5"
                        label={({ viewBox }) => {
                          const x = viewBox.x + viewBox.width - 10; // ปรับให้ label อยู่ขวาสุด
                          const y = viewBox.y - 10; // ยก label ขึ้นเหนือเส้น
                          return (
                            <g>
                              {/* Background ขาว */}
                              <rect
                                x={x - 50} // ปรับขนาดกล่อง
                                y={y - 14}
                                width={100}
                                height={20}
                                fill="#fff"
                      ...
                      >
                        ...
                        <td className="p-4">
                          <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-xl hover:from-blue-600 hover:to-blue-700 transition-all duration-200 transform hover:scale-105 font-medium shadow-md cursor-pointer"
                          >
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-3xl shadow-xl border border-white/20 mb-6 p-6">
          {loading ? (
            ...
            <DataTable
              data={Array.isArray(waterReq) ? waterReq : []}
