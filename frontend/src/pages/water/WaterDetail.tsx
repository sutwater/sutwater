...
const WaterMonitoringDashboard: React.FC = () => {
  ...
  waterDetail?.CameraDevice?.forEach((device: any) => {
    device.DailyWaterUsage?.forEach((usage: any) => {
      ...
      // ใช้ key เป็น deviceID เพื่อแยกเส้นกราฟ
      mergedData[date][`device_${device.ID}`] = usage.Usage;
    });
  });
  // ดึงค่า MeterValue และกรอง undefined
const waterMeterValues = waterDetail?.CameraDevice?.[0]?.WaterMeterValue
  ?.map(w => w.MeterValue)
  .filter((v): v is number => v !== undefined) || [];

// ถ้ามีค่า
const maxValue = waterMeterValues.length > 0 ? Math.max(...waterMeterValues) : null;
const minValue = waterMeterValues.length > 0 ? Math.min(...waterMeterValues) : null;
const avgValue = waterMeterValues.length > 0 
  ? waterMeterValues.reduce((a, b) => a + b, 0) / waterMeterValues.length 
  : null;
...
const dailyMeterData =
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div className="ml-auto text-right">
                <div className="text-2xl font-bold">{maxValue
                  ? `${maxValue} ลูกบาศก์เมตร`
                  : "ไม่มีข้อมูล"}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="ml-auto text-right">
                <div className="text-2xl font-bold">{minValue
                  ? `${minValue} ลูกบาศก์เมตร`
                  : "ไม่มีข้อมูล"}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="ml-auto text-right">
                <div className="text-2xl font-bold">{avgValue
                  ? `${avgValue} ลูกบาศก์เมตร`
                  : "ไม่มีข้อมูล"}
                </div>
              </div>
            </div>
  ...
  <div className="h-64 min-w-[700px]">
    <ResponsiveContainer width={dailyMeterData.length * 50} height="100%">
      <LineChart data={dailyMeterData}>
        ...
        <XAxis
          ...
          tick={{ fontSize: 12 }}
          stroke="#666"
          angle={-45}       // หมุน -45 องศา ให้อ่านง่าย
          textAnchor="end"
          interval={0}      // แสดงทุก tick
        />
        <YAxis tick={{ fontSize: 12 }} stroke="#666" />
        <Tooltip
  formatter={(value: any) => `${value} ลูกบาศก์เมตร`}
  labelFormatter={(label: string) => {
    const dateObj = new Date(label); // แปลง string เป็น Date
    return `วันที่: ${dateObj.toLocaleDateString('th-TH')} ${dateObj.toLocaleTimeString('th-TH')}`;
  }}
/>
